    ...
    async def update(
        ...
        *,
        ...
        try:
            ...
            print(f"[ProgBar] edit_text error: {e}")

    async def finish(self, text: str) -> None:
        try:
            await self.msg.edit_text(text, parse_mode="HTML")
        except Exception as e:
            print(f"[ProgBar] finish error: {e}")

    async def delete(self) -> None:
        try:
            await self.msg.delete()
        except Exception as e:
            print(f"[ProgBar] delete error: {e}")
    ...
    async def animate_loop(self, stop_event: asyncio.Event) -> None:
        ...
        while not stop_event.is_set():
            try:
                ...
                print(f"[ProgBar] animate_loop error: {e}")
...
MAX_TEXT_LENGTH = 50000  # 50 000 символов
MAX_IMAGES = 10
...
def _fallback_bibliography_for_topic(topic: str, subject: str, start: int = 1, limit: int = 12) -> str:
    """Резервный генератор ссылок на электронные каталоги (без хардкода зашитых авторов и DOI)."""
    ...
    lines = [
        f"Электронный каталог научных публикаций по теме «{topic}» [Электронный ресурс]. — URL: {u} (дата обращения: {today})."
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    paragraphs_target = max(3, round(chars / 600))  # минимум 3 абзаца
    ...
    base = (
        ...
        "   ❌ НЕЛЬЗЯ оставлять ссылку незавершённой!\n"
        "   📌 Если не знаешь страницу — НЕ СТАВЬ ССЫЛКУ.\n\n"
        "2️⃣ ОБЪЁМ (ЖЁСТКОЕ ПРАВИЛО):\n"
        ...
        "   • ЗАПРЕЩЕНО выходить за верхнюю границу.\n\n"
        "3️⃣ СТИЛЬ И ЯЗЫК:\n"
        f"   {style_instr}\n"
        "   • ЗАПРЕЩЕНЫ фразы-паразиты: «в современном мире», «необходимо отметить»,\n"
        "     «таким образом», «следует подчеркнуть», «играет важную роль».\n"
        "   • ТРЕБУЮТСЯ: конкретные факты, цифры, имена исследователей.\n\n"
        "4️⃣ ЗАГОЛОВКИ:\n"
        "   • БЕЗ точки после номера: «1 Название», «1.1 Название».\n"
        "   • БЕЗ точки в конце заголовка.\n"
        "   • Структурные элементы (ВВЕДЕНИЕ, ЗАКЛЮЧЕНИЕ) — без номера.\n\n"
        "5️⃣ ФОРМАТИРОВАНИЕ:\n"
        "   • ЗАПРЕЩЕН markdown (#, **, *, ```).\n"
        "   • Кавычки — только «ёлочки».\n"
        "   • Тире — длинное (—) с пробелами.\n"
        "   • Каждое предложение — завершённое.\n\n"
        "6️⃣ ПРОВЕРЬ СЕБЯ ПЕРЕД ОТПРАВКОЙ:\n"
        "   ✅ Все ссылки имеют вид [N, с. X]?\n"
        "   ✅ Нет оборванных ссылок?\n"
        "   ✅ Объём в допустимом диапазоне?\n"
        "   ✅ Нет запрещённых клише?\n"
        "   ✅ Нет markdown и опечаток?\n"
        "   ✅ Каждый абзац завершён мыслью?\n\n"
    ...
    if doc_type == "esse":
        return base + (
            ...
            "Не используй «мы рассмотрели», «было установлено»."
        )
    elif doc_type == "doklad":
        return base + (
            "\nЭТО ДОКЛАД — чётко, по делу, короткие абзацы. "
            "Ссылки ТОЛЬКО с номерами страниц."
        )
    elif doc_type == "article":
        return base + (
            "\nЭТО НАУЧНАЯ СТАТЬЯ — строгий академический язык, "
            "ссылки в каждом абзаце."
        )
    else:
        return base + (
            "\nЭТО АКАДЕМИЧЕСКАЯ РАБОТА — безличный стиль, "
            "ссылки в каждом абзаце (1–3 шт.)."
        )
...
async def generate_text_blocks(
    ...
    if verified_bib:
        style_sys += (
            ...
            f"{verified_bib}\n"
            "Внутритекстовые ссылки должны указывать на номера из этого списка и иметь вид [N, с. X] или [N]. "
            "Если точная страница неизвестна, используй формат [N] без выдумывания страниц."
        )

    # Если пользователь прислал свой список литературы — он приоритетнее
    user_lit = _source_data.get("user_literature", "") if _source_data else ""
    if user_lit:
        style_sys += (
    ...
    style_sys += (
        ...
        "предложения. Ссылки должны иметь вид `[N, с. P]` или `[N]`. "
        "Если точная страница неизвестна, используй `[N]` и НЕ выдумывай номер страницы. "
        "НИКОГДА не оставляй незавершённое `[N, с.` без номера. "
    ...
    async def _gen_one_block(key, prompt):
        ...
        if key == "literature" and verified_bib:
            ...
            if _rag_sources and key != "literature":
                is_file = bool(_source_data and _source_data.get("source_is_file"))
                if is_file:
                    strict_note = (
                        "\n\nКРАЙНЕ ВАЖНО: Используй информацию ИСКЛЮЧИТЕЛЬНО из предоставленного файла-источника. "
                        "Запрещено выдумывать внешние факты или добавлять стороннюю информацию, отсутствующую в файле.\n"
                        f"Содержимое файла-источника:\n{_rag_sources[:15000]}"
                    )
                else:
                    strict_note = (
                        "\n\nВАЖНО: Используй информацию из предоставленных ниже материалов как основу.\n"
                        f"Материалы пользователя:\n{_rag_sources[:5000]}"
                    )
                messages[-1]["content"] += strict_note
    ...
    user_lit = (_source_data.get("user_literature", "") if _source_data else "").strip()
    if user_lit:
        parts["literature"] = _normalize_bibliography(user_lit)
...
def _pseudo_page_for_source(n: str) -> str:
    """ОТКЛЮЧЕНО: выдуманные номера страниц.

    Раньше возвращалось str(12 + (int(n) * 17) % 150) — арифметическая выдумка,
    никак не связанная с реальным источником. Ссылка на несуществующую страницу
    проверяется мгновенно и является прямым основанием снять работу.
    Возвращаем пустую строку: ссылка останется в виде [N].
    """
    return ""
...
def _fallback_citation_page(n: str) -> str:
    """Возвращает пустую строку, чтобы не генерировать выдуманные страницы в ссылках."""
    return ""


def _nearest_page_from_map(page_map: dict) -> str:
    """ОТКЛЮЧЕНО: подстановка страницы чужого источника.

    Взять страницу источника №3 и приписать её источнику №5 — такая же выдумка,
    как сгенерировать номер арифметически.
    """
    return ""
...
async def _rewrite_if_ai_like(
    ...
    print(f"[HUMAN] Рерайт принят: {score} -> {new_score} (модель {used_model})")
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"[PIL Image Bounds Error] {e}")
...
# ── СЕМАФОР И ОЧЕРЕДЬ ДЛЯ LIBREOFFICE (защита сервера) ──
LO_SEMAPHORE = asyncio.Semaphore(2)  # максимум 2 конвертации параллельно
conversion_queue = asyncio.Queue()


async def convert_with_lo_safe(in_path: str, out_path: str, chat_id: int = 0) -> bool:
    """Безопасная конвертация через LibreOffice через очередь conversion_queue и семафор LO_SEMAPHORE."""
    loop = asyncio.get_running_loop()
    fut = loop.create_future()

    async def _on_complete(res: bool, job: dict):
        if not fut.done():
            fut.set_result(res)

    await conversion_queue.put({
        "in_path": in_path,
        "out_path": out_path,
        "chat_id": chat_id,
...
async def conversion_worker():
    """Фоновый воркер для асинхронной очереди конвертаций."""
    while True:
        job = await conversion_queue.get()
        try:
            in_path = job.get("in_path")
            out_path = job.get("out_path")
            chat_id = job.get("chat_id")
            res = await convert_with_lo_safe(in_path, out_path)
            if job.get("callback"):
                await job["callback"](res, job)
        except Exception as e:
            from error_logger import log_error
            log_error(stage="conversion_worker", message=str(e), user_id=job.get("chat_id", 0))
        finally:
            conversion_queue.task_done()


async def libreoffice_update_docx_async(in_path: str, out_path: str) -> bool:
    """Неблокирующее обновление TOC/полей через LibreOffice под семафором.

    FIX #2: вызовы ограничен LO_SEMAPHORE (макс. 2 процесса одновременно)
    чтобы избежать перегрузки процессора и оперативной памяти сервера.
    """
    soffice = shutil.which("soffice")
    if not soffice:
        return False
...
async def libreoffice_docx_to_pdf_async(in_path: str, out_dir: str) -> str | None:
    """Неблокирующая конвертация DOCX→PDF под семафором.

    Ограничено LO_SEMAPHORE для предотвращения забивания ресурсов CPU/RAM.
    """
    soffice = shutil.which("soffice")
    if not soffice:
        return None
...
async def h_document(message: Message, state: FSMContext) -> None:
    ...
    if document and document.file_size and document.file_size > MAX_FILE_SIZE:
        await message.answer("❌ Файл слишком большой (максимум 20 МБ)", parse_mode="HTML")
        return
...
async def h_model(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        ...
        f"└─────────────────────────\n\n"
        f"⚠️ <i>Обратите внимание: работа генерируется ИИ, не является готовой на 100% и требует вашей обязательной проверки перед сдачей.</i>\n\n"
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        await event.answer(
            "✅ <b>Работа готова!</b>\n\n"
            "<b>⚠️ ВАЖНО: РАБОТА ТРЕБУЕТ ПРОВЕРКИ!</b>\n"
            "<b>Сгенерированный документ не является на 100% готовой работой. ИИ может допускать ошибки в фактах, расчётах и оформлении. Обязательно проверьте и отредактируйте материал перед использованием!</b>\n\n"
...
async def main() -> None:
    ...
    asyncio.create_task(conversion_worker())
