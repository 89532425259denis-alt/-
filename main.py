...
_AI_MARKER_REPLACEMENTS = [
    ...
    # ── Расширенные ИИ-маркеры (v2) ──
    (r"(?i)\bследует также отметить,?\s+что\s+", ""),
    (r"(?i)\bстоит подчеркнуть,?\s+что\s+", ""),
    (r"(?i)\bв настоящее время\s+", ""),
    (r"(?i)\bв различных областях\s+", ""),
    (r"(?i)\bширокий спектр\s+", ""),
    (r"(?i)\bширокий диапазон\s+", ""),
    (r"(?i)\bкомплексный подход\b", "подход"),
    (r"(?i)\bкомплексный анализ\b", "анализ"),
    (r"(?i)\bв контексте данного\b", "в данном"),
    (r"(?i)\bв контексте этой\b", "в этой"),
    (r"(?i)\bв рамках настоящего\b", "в данном"),
    (r"(?i)\bв свете вышесказанного,?\s+", ""),
    (r"(?i)\bподводя черту,?\s+", ""),
    (r"(?i)\bрезюмируя сказанное,?\s+", ""),
    (r"(?i)\bобобщая вышесказанное,?\s+", ""),
    (r"(?i)\bучитывая вышеизложенное,?\s+", ""),
    (r"(?i)\bопираясь на вышеприведённое,?\s+", ""),
    (r"(?i)\bпредставляется возможным\b", "можно"),
    (r"(?i)\bпредставляет собой\b", "является"),
    (r"(?i)\bиграет важную роль\b", "важна"),
    (r"(?i)\bимеет важное значение\b", "важна"),
    (r"(?i)\bзанимает особое место\b", "важна"),
    (r"(?i)\bнельзя не учитывать,?\s+что\s+", ""),
    (r"(?i)\bнеобходимо учитывать,?\s+что\s+", ""),
    (r"(?i)\bважно учитывать,?\s+что\s+", ""),
    (r"(?i)\bнет сомнения,?\s+что\s+", ""),
    (r"(?i)\bстоит отметить,?\s+что\s+", ""),
    (r"(?i)\bнелишним будет отметить,?\s+что\s+", ""),
    (r"(?i)\bбудет уместно отметить,?\s+что\s+", ""),
    (r"(?i)\bследует обратить внимание,?\s+что\s+", ""),
    (r"(?i)\bнадо отметить,?\s+что\s+", ""),
    (r"(?i)\bнужно отметить,?\s+что\s+", ""),
    (r"(?i)\bважно отметить,?\s+что\s+", ""),
    (r"(?i)\bпринимая во внимание,?\s+", ""),
    (r"(?i)\bпринимая во внимание тот факт,?\s+что\s+", ""),
    (r"(?i)\bвсе вышеперечисленное\s+", ""),
    (r"(?i)\bвсе вышеперечисленные\s+", ""),
    (r"(?i)\bсреди которых следует выделить\s+", "в частности, "),
    (r"(?i)\bнаряду с этим\b", "также"),
    (r"(?i)\bв дополнение к вышесказанному\b", "также"),
    (r"(?i)\bвместе с тем\b", "однако"),
    (r"(?i)\bв то же время\b", "однако"),
    (r"(?i)\bкомплексная оценка\b", "оценка"),
    (r"(?i)\bкомплексное исследование\b", "исследование"),
    (r"(?i)\bкомплексное изучение\b", "изучение"),
    (r"(?i)\bактуальность исследования обусловлена\b", "Исследование актуально"),
    (r"(?i)\bнаучная новизна исследования заключается в\b", "Новизна работы состоит в"),
    (r"(?i)\bстепень научной разработанности проблемы\b", "научная разработанность темы"),
    (r"(?i)\bстепень разработанности проблемы\b", "научная разработанность темы"),
]
...
async def h_user_literature(message: Message, state: FSMContext) -> None:
    ...
    if len(content) < 20:
        await message.answer(
            "❌ Список литературы слишком короткий. Вставьте хотя бы 3–5 источников.",
            parse_mode="HTML", reply_markup=kb_back_cancel(),
        )
        return
    await state.update_data(user_literature=content)
    await message.answer(
        f"✅ Список литературы принят ({len(content)} символов, ~{len([l for l in content.split(chr(10)) if l.strip()])} позиций).\n\n"
        "🏛 <b>Тип учебного заведения:</b>",
        reply_markup=with_back(kb_institution()),
        parse_mode="HTML",
    )
